...
WIDTH, HEIGHT = 800, 600  # Default values, won't be used in fullscreen mode
WHITE = (255, 255, 255)
RED = (255, 0, 0)
BLUE = (0, 0, 255)
GREEN = (0, 255, 0)
...
class Tank:
    ...
    def move(self, keys_pressed):
        ...
        # Prevent tank from moving out of bounds (add the tank's width and height to avoid clipping)
        if 0 + TANK_WIDTH / 2 <= new_x <= WIDTH - TANK_WIDTH / 2:
            self.x = new_x
        if 0 + TANK_HEIGHT / 2 <= new_y <= HEIGHT - TANK_HEIGHT / 2:
            self.y = new_y
...
def draw_retry_button():
    ...
    pygame.draw.rect(screen, DARK_GRAY, button_rect, 3)  # Add dark gray border
...
def draw_exit_button():
    ...
    button_rect = pygame.Rect(WIDTH // 2 - 60, HEIGHT // 2 + 120, 120, 50)
    pygame.draw.rect(screen, RED, button_rect, border_radius=15)
    pygame.draw.rect(screen, DARK_GRAY, button_rect, 3)  # Add dark gray border
...
def draw_game_over_text():
    ...
    text_shadow = font.render("Game Over", True, (150, 150, 150))  # Shadow effect
    screen.blit(text_shadow, (WIDTH // 2 - 100 + 4, HEIGHT // 2 - 50 + 4))  # Shadow offset
...
def main():
    ...
    while run:
        ...
        if tank1.hit_count >= 5 or tank2.hit_count >= 5:
            ...
            # Wait for the player to click on buttons
